  ...
  Suggestions.prototype = {
    ...
    open: function() {
      var data       = {},
          ...
          inputValue = options.inputProcessor(trim(this.input.val()));
    ...
    _step: function(dir) {
      ...
      if (selected.length) {
        ...
        selected = ul.find('li:' + (dir === 1 ? 'first' : 'last')).addClass('selected');
      }
      this._checkVisibility(selected);
    },
    ...
    _checkVisibility: function(selected) {
      if (!selected.length) { return; }
      var selectedPosition = selected.position(),
          bottom           = selectedPosition.top + selected.height(),
          containerHeight  = this.container.height(),
          scrollPosition   = this.container.scrollTop();
      if (bottom > containerHeight) {
        this.container.scrollTop(scrollPosition + (bottom - containerHeight));
      } else if (selectedPosition.top < 0) {
        this.container.scrollTop(scrollPosition + selectedPosition.top);
      }
    },
    ...
    resetOpenInterval: function(reopenNow) {
      var self = this;
      clearInterval(this._delayInterval);
  ...
  $.fn.autocompletr = function(options) {
    if (typeof options === 'string') {
      ...
      options = $.extend({
        ...
        delay: 300,
        // time till suggestions disappear after blur (minimum is 200 — else it can't be guaranteed that your clicks on a suggestion
        // will be considered)
        closeDelay: 500,
        ...
        matcher: function(input, source) {
          ...
          var reg = regExes[input] = regExes[input] || new RegExp($.map((trim(input) || "").split(""), function(value) {
            // Escape regex chars
        ...
        // determines if last suggestions will be reopened if user focus the input field again
        reopenOnFocus: false,
    ...
    var thisElements = this.each(function() {
      ...
      element
        ...
        .on('focus.autocomplete', function(event) {
          suggestions.resetOpenInterval(options.reopenOnFocus);
        })
        .on('blur.autocomplete', function(event) {
          ...
          setTimeout(function() {
            suggestions.close();
          }, Math.max(200, options.closeDelay));
        });
    });
